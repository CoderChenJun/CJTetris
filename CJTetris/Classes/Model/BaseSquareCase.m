...
-(void)setButtonState:(BOOL)states
{
    ...
    [self setButtonTag:(case1.x + case1.y * 100 + 10000) states:states];
    [self setButtonTag:(case2.x + case2.y * 100 + 10000) states:states];
    [self setButtonTag:(case3.x + case3.y * 100 + 10000) states:states];
    [self setButtonTag:(case4.x + case4.y * 100 + 10000) states:states];
...
//        return NO;
    ...
    //1.2.1根据所传参数设置按钮显示状态：显示or隐藏.隐藏取反为NO,只要满足一个条件
    if (x < 0 || x >= self.gameViewNumX || y > self.gameViewNumY || ![self getButtonShowStateX:x Y:y])
        return YES;
    else
        return NO;
...
#pragma mark - 保证方块从最顶部开始出现
    ...
    UIButton * button = (UIButton *)[rvc.view viewWithTag:(x + y * 100 + 10000)];
